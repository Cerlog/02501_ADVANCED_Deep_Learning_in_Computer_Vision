...
class Diffusion:
    ...
    def get_betas(self, schedule='linear'):
        if schedule == 'linear':
            ...
            return torch.tensor([self.beta_end + 0.5 * (self.beta_start - self.beta_end) * (1 + torch.cos(
                t * 3.141592653589793 / self.T)) for t in range(self.T)], device=self.device)
        elif schedule == 'log':
            return torch.tensor([self.beta_end + (self.beta_start - self.beta_end) * torch.log(1 + t) / torch.log(1 + self.T) for t in range(self.T)], device=self.device
            )
        elif schedule == 'exp':
            return torch.tensor([self.beta_end + (self.beta_start - self.beta_end) * (1 - torch.exp(-5 * t / self.T)) for t in range(self.T)], device=self.device)
        else :
